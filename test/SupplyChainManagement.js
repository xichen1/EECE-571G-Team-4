...
describe("SupplyChainManagement", function () {
  ...
  beforeEach(async function () {
    ...
    [admin, manufacturer, logistics, retailer, consumer] =
    ...
    await supplyChainContract.grantRole(
      MANUFACTURER_ROLE,
      manufacturer.address
    );
  ...
  it("1. Should allow a manufacturer to create a product and have correct inventory of the product", async function () {
    ...
    const createProductTx = await supplyChainContract
      .connect(manufacturer)
      .createProduct(productId, productName, productPrice, productQty);
  ...
  it("2. A retailer cannot order products from the manufacturer if the product is not available", async function () {
    ...
    await expect(
      supplyChainContract.connect(retailer).orderProduct(1, 10)
  ...
  it("3. A retailer cannot order products from the manufacturer if order quantity is larger than manufacturer's inventory", async function () {
    ...
    await supplyChainContract
      .connect(manufacturer)
      .createProduct(1, "Test Product", ethers.parseEther("1"), 100);

    // Retailer tries to order 150 products, which is more than available
    await expect(
      supplyChainContract.connect(retailer).orderProduct(1, 150)
  ...
  it("4. A retailer cannot order products from the manufacturer if he/she does not send enough ether", async function () {
    ...
    await expect(
      supplyChainContract
        .connect(retailer)
        .orderProduct(1, 10, { value: ethers.parseEther("0.5") })
  ...
  it("5. A retailer can order products from the manufacturer if everything is correct", async function () {
    ...
    await expect(
      supplyChainContract
        .connect(retailer)
        .orderProduct(productId, orderQty, { value: orderValue })
    )
  ...
  it("6. A manufacturer cannot make a distribution request if there is no order been made", async function () {
    ...
    await expect(
      supplyChainContract
        .connect(manufacturer)
        .requestDistribution(productId, productQty)
  ...
  it("7. A manufacturer cannot distribute products when the distribution quantity is more than the inventory", async function () {
    ...
    await expect(
      supplyChainContract
        .connect(manufacturer)
        .requestDistribution(productId, distributionQty)
  ...
  it("8. A logistic provider cannot ship products if products are not ready for shipment", async function () {
    ...
    await expect(
      supplyChainContract.connect(logistics).shipProduct(productId, orderQty)
  ...
  it("9. A manufacturer's inventory should be correct after a logistic provider has shipped the product", async function () {
    ...
    await supplyChainContract
      .connect(manufacturer)
      .requestDistribution(productId, orderQty);

    // Logistic provider ships the product
    await supplyChainContract
      .connect(logistics)
      .shipProduct(productId, orderQty);

    // Check the manufacturer's inventory for the correct quantity after shipping
    const productAfterShipping =
      await supplyChainContract.manufacturerInventory(productId);
  ...
  it("10. A retailer cannot receive products if products are not shipped", async function () {
    ...
    await expect(
      supplyChainContract.connect(retailer).receiveProduct(productId)
  ...
  it("11. A retailer's inventory of the product should be correct after receiving the product", async function () {
    ...
    const retailerProduct = await supplyChainContract.retailerInventory(
      productId
    );
  ...
  it("12. A retailer cannot list products for sale if the quantity of the product is 0", async function () {
    ...
    await expect(
      supplyChainContract
        .connect(retailer)
        .listProductForSale(productId, productPrice)
  ...
  it("13. A retailer can list products for sale and set the price if everything is correct", async function () {
    ...
    await supplyChainContract
      .connect(retailer)
      .listProductForSale(productId, salePrice);

    // Verify the product is now listed for sale at the correct price
    const listedProduct = await supplyChainContract.retailerInventory(
      productId
    );
  ...
  it("14. A consumer cannot purchase a product if there's no product listed for sale", async function () {
    ...
    await expect(
      supplyChainContract
        .connect(consumer)
        .purchaseProduct(productId, purchaseQty, { value: purchaseValue })
    ).to.be.revertedWith("Product is not for sale"); // This message should match the revert message in your smart contract

    const consumerPurchase = await supplyChainContract.consumerPurchases(
      consumer.address,
      productId
    );
  ...
  it("15. A consumer cannot purchase products more than the inventory of the retailer", async function () {
    ...
    await supplyChainContract
      .connect(manufacturer)
      .createProduct(productId, productName, productPrice, initialProductQty);
  ...
  it("16. A consumer cannot purchase products from the retailer if he/she does not send enough ether", async function () {
    ...
    await supplyChainContract
      .connect(retailer)
      .orderProduct(productId, orderQty, { value: productPrice * orderQty });
    ...
    await expect(
      supplyChainContract
        .connect(consumer)
        .purchaseProduct(productId, purchaseQty, {
          value: insufficientValue * purchaseQty,
        })
  ...
  it("17. A consumer should purchase products from the retailer if everything is correct", async function () {
    ...
    await supplyChainContract
      .connect(retailer)
      .orderProduct(productId, orderQty, {
        value: manufacturerPrice * orderQty,
      });
    ...
    const manufacturerProduct = await supplyChainContract.manufacturerInventory(
      productId
    );
  ...
  it("18. A consumer can verify the product authenticity and availability", async function () {
    ...
    let [isAuthentic, isAvailableForPurchase, price] =
      await supplyChainContract.verifyProductAuthenticity(productId);
    ...
    [isAuthentic, isAvailableForPurchase, price] =
      await supplyChainContract.verifyProductAuthenticity(productId);
    expect(isAuthentic).to.be.true;
    ...
    [isAuthentic, isAvailableForPurchase] =
      await supplyChainContract.verifyProductAuthenticity(fakeProductId);
  ...
  it("19. Should use getAllManufacturerProducts to return all manufacturer products correctly", async function () {
    ...
    let tx = await supplyChainContract
      .connect(manufacturer)
      .createProduct(1, "Product 1", ethers.parseEther("0.01"), 100);
    await tx.wait(); // Wait for the transaction to be mined

    tx = await supplyChainContract
      .connect(manufacturer)
      .createProduct(2, "Product 2", ethers.parseEther("0.02"), 200);
    await tx.wait(); // Wait for the transaction to be mined
  ...
  it("20. Should correctly return all products in the retailer's inventory", async function () {
    ...
    await supplyChainContract
      .connect(retailer)
      .orderProduct(product1Id, quantity1, {
        value: manufacturerPrice1 * quantity1,
      });
    await supplyChainContract
      .connect(retailer)
      .orderProduct(product2Id, quantity2, {
        value: manufacturerPrice2 * quantity2,
      });

    // Manufacturer marks the product as ready for shipment
    await supplyChainContract
      .connect(manufacturer)
      .requestDistribution(product1Id, quantity1);
    await supplyChainContract
      .connect(manufacturer)
      .requestDistribution(product2Id, quantity2);

    // Logistic provider ships the product
    await supplyChainContract
      .connect(logistics)
      .shipProduct(product1Id, quantity1);
    await supplyChainContract
      .connect(logistics)
      .shipProduct(product2Id, quantity2);
    ...
    await supplyChainContract
      .connect(retailer)
      .listProductForSale(product1Id, salePrice1);
    await supplyChainContract
      .connect(retailer)
      .listProductForSale(product2Id, salePrice2);
  ...
  it("21. Should use getAllConsumerPurchases to return all purchases made by a specific consumer correctly", async function () {
    ...
    await supplyChainContract
      .connect(consumer)
      .purchaseProduct(product1Id, purchaseQty1, { value: purchaseValue1 });
    await supplyChainContract
      .connect(consumer)
      .purchaseProduct(product2Id, purchaseQty2, { value: purchaseValue2 });

    // Verify the consumer's purchase history
    const purchaseList = await supplyChainContract.getAllConsumerPurchases(
      consumer
    );
